...
* The result of dividing this arbitrary-precision decimal
 ...
 Divide overload that takes an <code>EContext</code> (such as <code>
 EContext.Decimal128</code>) instead.
...
* <code>ctx</code> - An arithmetic context object to control the precision and
 ...
 ignored. If <code>HasFlags</code> of the context is true, will also store the
 flags resulting from the operation (the flags are in addition to the
 pre-existing flags).
...
* Returns the next value that is closer to the other object' s value
 ...
 parameter <code>ctx</code> is null, the precision is 0, or <code>ctx</code> has an
...
* The closest value to this object's value, rounded to the specified
 precision. If <code>ctx</code> is null or the precision and exponent range are
...
* <code>ctx</code> - An arithmetic context to control precision and rounding of the
 result. If <code>HasFlags</code> of the context is true, will also store the
 flags resulting from the operation (the flags are in addition to the
 pre-existing flags). Can be null, in which case the default rounding mode is
 HalfEven.
...
* <code>otherValue</code> - An arbitrary-precision decimal number containing the
 ...
 desired exponent of 3: <code>10e3</code>, <code>8888e3</code>, <code>4.56e5</code>.
...
* <code>ctx</code> - An arithmetic context object to control the precision, rounding,
 ...
 If <code>HasFlags</code> of the context is true, will also store the flags
...
* <code>ctx</code> - An arithmetic context object to control the precision, rounding,
 ...
 division. If <code>HasFlags</code> of the context is true, will also store the
 flags resulting from the operation (the flags are in addition to the
...
* <code>ctx</code> - An arithmetic context object to control the precision. The
 ...
 rounding mode is always treated as HalfEven). If <code>HasFlags</code> of the
...
Returns an arbitrary-precision decimal number with the same value as this
 object but rounded to an integer, without adding the <code>FlagInexact</code> or
 <code>FlagRounded</code> flags. The resulting number's Exponent property will not
 necessarily be 0; use the Quantize method instead to give the result an
 exponent of 0.
...
* <code>ctx</code> - An arithmetic context to control precision and rounding of the
 ...
 FlagRounded</code> and <code>FlagInexact</code> flags (the only difference between this
...
* The closest value to this object's value, rounded to the specified
 precision. Returns the same value as this object if <code>ctx</code> is null or
...
* A number whose exponent is increased by <code>places</code>. For example,
 if <code>places</code> is 5, "78E-2" becomes "78E+3" and has a bigger value.
...
* A number whose exponent is generally increased by <code>places</code>.
 For example, in general, if <code>places</code> is 5, "78E-2" becomes "78E+3" and
 has a bigger value.
...
* A number whose exponent is increased by <code>bigPlaces</code>. For
 example, if <code>bigPlaces</code> is 5, "78E-2" becomes "78E+3" and has a bigger
 value.
...
* A number whose exponent is generally increased by <code>bigPlaces</code>.
 For example, in general, if <code>bigPlaces</code> is 5, "78E-2" becomes "78E+3"
 and has a bigger value.

**Throws:**

* <code>NullPointerException</code> - The parameter <code>bigPlaces</code> is null.
...
* The square root. Signals the flag FlagInvalid and returns NaN if
 ...
 (NaN) if the parameter <code>ctx</code> is null or the precision is unlimited
...
* <code>UnsupportedOperationException</code> - There is not enough memory to store the value
 as an EInteger.
...
* <code>ec</code> - An arithmetic context to control the precision, rounding, and
 ...
 this parameter is <code>EContext.Binary64</code>. Can be null.
