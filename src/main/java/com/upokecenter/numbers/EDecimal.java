...
*/
   ...
   * implementations of {@code float} and {@code double} , including in C# and
   ...
   * {@code http://speleotrove.com/decimal/decarith.html}).</p> <p><b>Errors and
   ...
   * use of a 32-bit integer internally as a length), the operation may signal an
   ...
   * reasons:</p> <ul><li>{@code EDecimal} objects are immutable, so they can't
   ...
   * "Floating-Point Determinism" ({@code https://randomascii.wordpress.com/}
   * {@code 2013/07/16/floating-point-determinism/}), identified many
   ...
   * (except for methods that convert to or from {@code float} , {@code double} ,
   ...
   * precisions (problems present in C and C++, for example) don't exist with
   ...
   * The next 8 bits are the exponent area.</li><li>C. If the highest bit is one,
  ...
  public final class EDecimal implements Comparable<EDecimal> {
     ...
     * EDecimal.FromDouble(0.1)}), since not all decimal numbers can be converted
     ...
     * EDecimal.FromString("0.1")}). </p> <p>The input value can be a not-a-number
     ...
     * EDecimal.FromDouble(Double.NaN)} could produce an object that is represented
...
@Deprecated
     ...
     * results when passing a literal decimal number (for example, calling {@code
     * EDecimal.FromSingle(0.1f)}), since not all decimal numbers can be converted
     * to exact binary numbers (in the example given, the resulting
     ...
     * EDecimal.FromSingle(Float.NaN)} or {@code EDecimal.FromSingle(Float.NaN)}
     * could produce an object that is represented differently between DotNet and
     * Java, because {@code Float.NaN} / {@code Float.NaN} may have a different
...
(1 << (EFloat.Binary32SignifAreaBits - 1))) != 0;
    ...
    /**
     * <p>Creates a decimal floating-point number from a binary floating-point
...
(1 << (EFloat.Binary16SignifAreaBits - 1))) != 0;
     ...
     * EContext)} for more information. Note that calling the overload that takes
     ...
     * context specifies a precision limit and exponent range.</p>
     * @return <p>An arbitrary-precision decimal number with the same value as the
     * given sequence of {@code char} s.</p>
    ...
    /**
     * <p>Creates an arbitrary-precision decimal number from a sequence of {@code
     * char} s that represents a number.</p> <p>The format of the sequence
     ...
     * FromString(string, int, int, EContext)} for more information. Note that
     ...
     * positive).</p>
     * @return <p>An arbitrary-precision decimal number. Returns signaling NaN if
     * this value is signaling NaN. (In this sense, this method is similar to the
     ...
     * {@code EContext} (such as {@code EContext.Decimal128}) instead.</p>
     *
     */
     ...
     * dividend are 0; or, either {@code ctx} is null or {@code ctx} 's precision
...
@Deprecated
     ...
     * Decimal Arithmetic Specification, any rounding mode is allowed.).</p>
    ...
    /**
     * <p>Adds this arbitrary-precision decimal floating-point number and a 64-bit
    ...
    /**
     * <p>Subtracts a 64-bit signed integer from this arbitrary-precision decimal
    ...
    /**
     * <p>Multiplies this arbitrary-precision decimal floating-point number by a
    ...
    /**
     * <p>Divides this arbitrary-precision decimal floating-point number by a
     * 64-bit signed integer and returns the result; returns NaN instead if the
     ...
     * divisor (this arbitrary-precision decimal floating-point number) is 0 and
     * the dividend (the other 64-bit signed integer) is nonzero. Returns
     * not-a-number (NaN) if the divisor and the dividend are 0. Returns NaN if the
     * result can't be exact because it would have a nonterminating decimal
     * expansion (examples include 1 divided by any multiple of 3, such as 1/3 or
     * 1/12). If this is not desired, use DivideToExponent instead, or use the
     * Divide overload that takes an {@code EContext} (such as {@code
     * EContext.Decimal128}) instead.</p>
     *
     */
    ...
    /**
     * <p>Adds this arbitrary-precision decimal floating-point number and a 32-bit
     * signed integer and returns the result. The exponent for the result is the
     * lower of this arbitrary-precision decimal floating-point number's exponent
    ...
    /**
     * <p>Subtracts a 32-bit signed integer from this arbitrary-precision decimal
     * floating-point number and returns the result. The exponent for the result is
     * the lower of this arbitrary-precision decimal floating-point number's
    ...
    /**
     * <p>Divides this arbitrary-precision decimal floating-point number by a
     * 32-bit signed integer and returns the result; returns NaN instead if the
     * result would have a nonterminating decimal expansion (including 1/3, 1/12,
     * 1/7, 2/3, and so on); if this is not desired, use DivideToExponent, or use
     * the Divide overload that takes an EContext.</p>
     ...
     * divisor (this arbitrary-precision decimal floating-point number) is 0 and
     * the dividend (the other 32-bit signed integer) is nonzero. Returns
     * not-a-number (NaN) if the divisor and the dividend are 0. Returns NaN if the
     * result can't be exact because it would have a nonterminating decimal
     * expansion (examples include 1 divided by any multiple of 3, such as 1/3 or
     * 1/12). If this is not desired, use DivideToExponent instead, or use the
     ...
     * reversed.</p>
     * @return <p>An arbitrary-precision decimal number. If this value is positive
     * zero, returns negative zero. Returns signaling NaN if this value is
     * signaling NaN. (In this sense, this method is similar to the "copy-negate"
     * operation in the General Decimal Arithmetic Specification, except this
     * method does not necessarily return a copy of this object.).</p>
     *
     */
     ...
     * precision is unlimited and rounding isn't needed.</p>
     * @return <p>An arbitrary-precision decimal number. If this value is positive
     * zero, returns positive zero. Signals FlagInvalid and returns quiet NaN if
     * this value is signaling NaN.</p>
     *
     */
    ...
    /**
     * <p>Returns an arbitrary-precision decimal number with the same value but a
     * new exponent. </p><p>Note that this is not always the same as rounding to a
     * given number of decimal places, since it can fail if the difference between
     * this value's exponent and the desired exponent is too big, depending on the
     * maximum precision. If rounding to a number of decimal places is desired,
     * it's better to use the RoundToExponent and RoundToIntegral methods
     * instead.</p> <p><b>Remark:</b> This method can be used to implement
     * fixed-point decimal arithmetic, in which each decimal number has a fixed
     * number of digits after the decimal point. The following code example returns
     * a fixed-point number with up to 20 digits before and exactly 5 digits after
     * the decimal point:</p> <pre> /* After performing arithmetic operations,
     * adjust /* the number to 5&#x2a;&#x2f;&#x2a;&#x2f; /*&#x2a;&#x2f; digits after the decimal point number =
     * number.Quantize(EInteger.FromInt32(-5), /* five digits after the decimal
    ...
    /**
     * <p>Returns an arbitrary-precision decimal number with the same value as this
     * one but a new exponent. </p> <p><b>Remark:</b> This method can be used to
     * implement fixed-point decimal arithmetic, in which a fixed number of digits
    ...
    /**
     * <p>Returns an arbitrary-precision decimal number with the same value as this
     * object but with the same exponent as another decimal number. </p><p>Note
     ...
     * and the desired exponent is too big, depending on the maximum precision. If
     * rounding to a number of decimal places is desired, it's better to use the
     * RoundToExponent and RoundToIntegral methods instead.</p> <p><b>Remark:</b>
     ...
     * decimal floating-point number. The remainder is the number that remains when
     * the absolute value of this arbitrary-precision decimal floating-point number
     ...
     * flags resulting from the operation (the flags are in addition to the
     * pre-existing flags). Can be null, in which the precision is unlimited.</p>
     * @return <p>The remainder that would result when this arbitrary-precision
     * decimal floating-point number is divided by another arbitrary-precision
     * decimal floating-point number. Signals FlagDivideByZero and returns infinity
     * if the divisor (this arbitrary-precision decimal floating-point number) is 0
     ...
     * value representable in the given precision. If the result can't fit the
     ...
     * an exponent range, the new exponent must be changed to the given exponent
     ...
     * rounding mode is HalfEven.</p>
     * @return <p>An arbitrary-precision decimal number rounded to the closest
     * value representable in the given precision. Signals FlagInvalid and returns
     * not-a-number (NaN) if the result can't fit the given precision without
     ...
     * rounding mode is HalfEven.</p>
     * @return <p>An arbitrary-precision decimal number rounded to the closest
     * integer representable in the given precision. Signals FlagInvalid and
     * returns not-a-number (NaN) if the result can't fit the given precision
     * without rounding. Signals FlagInvalid and returns not-a-number (NaN) if the
     * arithmetic context defines an exponent range, the new exponent must be
     ...
     * mode is HalfEven.</p>
     * @return <p>An arbitrary-precision decimal number rounded to the closest
     * integer representable in the given precision. If the result can't fit the
     * precision, additional digits are discarded to make it fit. Signals
     * FlagInvalid and returns not-a-number (NaN) if the arithmetic context defines
     * an exponent range, the new exponent must be changed to 0 when rounding, and
     * 0 is outside of the valid range of the arithmetic context.</p>
     *
     */
...
@Deprecated
     ...
     * addition to the pre-existing flags). Can be null, in which case the
     * precision is unlimited and no rounding is needed.</p>
     * @return <p>A number whose exponent is generally increased by {@code places}.
     * For example, in general, if {@code places} is 5, "78E-2" becomes "78E+3" and
     * has a bigger value.</p>
     *
     */
     ...
     * addition to the pre-existing flags). Can be null, in which case the
     * precision is unlimited and no rounding is needed.</p>
     * @return <p>A number whose exponent is generally increased by {@code
     * bigPlaces}. For example, in general, if {@code bigPlaces} is 5, "78E-2"
     * becomes "78E+3" and has a bigger value.</p>
     ...
     * (Unlike in the General Decimal Arithmetic Specification, any rounding mode
     * is allowed.).</p>
...
@Deprecated
     ...
     * mode. </p> <p>If this value is a NaN, sets the high bit of the binary64
     * value's significand area for a quiet NaN, and clears it for a signaling NaN.
     * Then the other bits of the significand area are set to the lowest bits of
     * this object's unsigned significand, and the next-highest bit of the
     * significand area is set if those bits are all zeros and this is a signaling
     * NaN.</p>
     * @return <p>The closest 64-bit floating-point number to this value, encoded
     * in the IEEE 754 binary64 format. The return value can be positive infinity
     * or negative infinity, encoded in the IEEE 754 binary64 format, if this value
    ...
    /**
     * <p>Converts this value to its closest equivalent as a 64-bit floating-point
     * number, using the half-even rounding mode. </p> <p>If this value is a NaN,
     * sets the high bit of the 64-bit floating point number's significand area for
    ...
    /**
     * <p>Converts this value to an arbitrary-precision integer, discarding the
     * fractional part in this value. Note that depending on the value, especially
     ...
     * (NaN).</p>
     * @throws UnsupportedOperationException <p>There is not enough memory to store the
     * value as an EInteger.</p>
     *
     */
     ...
     * the fractional part of the value would be lost. Note that depending on the
     * value, especially the exponent, generating the arbitrary-precision integer
     * may require a huge amount of memory. Use the ToSizedEIntegerIfExact method
     * to convert a number to an EInteger only if the integer fits in a bounded bit
...
@Deprecated
    ...
    /**
     * <p>Creates a binary floating-point number from this object's value. Note
     ...
     * value, using the half-even rounding mode.</p>
...
@Deprecated
    ...
    /**
     * <p>Converts this value to a string as though with the toString method, but
     * without using exponential notation.</p>
     * @return <p>A text string.</p>
     *
     */
...
(1 << (EFloat.Binary16Bits - 1)))) :
     ...
     * mode. </p> <p>If this value is a NaN, sets the high bit of the 32-bit
     * floating point number's significand area for a quiet NaN, and clears it for
     * a signaling NaN. Then the other bits of the significand area are set to the
     * lowest bits of this object's unsigned significand, and the next-highest bit
     * of the significand area is set if those bits are all zeros and this is a
     * signaling NaN.</p>
     * @return <p>The closest 32-bit binary floating-point number to this value,
     * encoded in the IEEE 754 binary32 format. The return value can be positive
     * infinity or negative infinity if this value exceeds the range of a 32-bit
     * floating point number.</p>
     *
     */
...
(1 << (EFloat.Binary32Bits - 1)))) :
    ...
    /**
     * <p>Converts this value to a text string. Returns a value compatible with
     * this class's FromString method.</p>
     * @return <p>A string representation of this object. The text string will be
     * in exponential notation (expressed as a number 1 or greater, but less than
     * 10, times a power of 10) if this object's Exponent property is greater than
     * 0 or if the number's first nonzero decimal digit is more than five digits
     ...
     * @return <p>The given arbitrary-precision decimal number plus one.</p>
     *
     */
     ...
     * @return <p>The given arbitrary-precision decimal number minus one.</p>
     *
     */
