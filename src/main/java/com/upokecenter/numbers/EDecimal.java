 ...
 */
  ...
  * https://randomascii.wordpress.com/2013/07/16/floating-point-determinism/),
   ...
   * and here is how they relate to the EDecimal and EFloat classes of this
   * library:</p> <ul><li>Runtime floating-point settings: All the settings
   * that change how EDecimal and EFloat behave are given as parameters to
   * the appropriate methods, especially via EContext objects, which
   * specify the precision, rounding, and exponent range of numbers, among
   * other things. The EDecimal and EFloat classes avoid the use of
   *  "native" floating-point data types (except for methods that convert to
   * or from <code>float</code> , <code>double</code> , or <code>System.Decimal</code>).
   *  Such "native" types are often subject to runtime settings that change
   * how floating-point math behaves with them, and these settings are
   * often not accessible to .NET or Java code.</li> <li>Non-associativity
   * and intermediate precisions: In general, EDecimal and EFloat use
    ...
    private static int CompareEDecimalToEFloat(EDecimal ed, EFloat ef) {
      ...
      if (ef.getExponent().compareTo(EInteger.FromInt64(-1000)) < 0) {
        ...
        if (
          ...
          // System.out.println("taexp={0}, oaexp={1} ratio={2}"
...
@Deprecated
    ...
    public EFloat ToEFloat(EContext ec) {
      ...
      if (bigintExp.signum() > 0) {
        ...
        // System.out.println("scale=" + scale + " mantissaPrecision=" +
        ...
        if (ec == null || !ec.getHasMaxPrecision()) {
          ...
          if (num.GetUnsignedBitLengthAsEInteger().compareTo(10000) <= 0 &&
            ...
            // System.out.println("num=" + (num.Divide(gcd)));
        ...
        if (ec == null || !ec.getHasMaxPrecision()) {
          ...
          do {
            ...
            // System.out.println("quorem[0]="+quorem[0]);
            // System.out.println("quorem[1]="+quorem[1]);
            // System.out.println("desiredLow="+desiredLow);
            // System.out.println("desiredHigh="+desiredHigh);
        ...
        EFloat efret = this.WithThisSign(
            EFloat.Create(
              quorem[0],
              adjust.AsEInteger()));
        // System.out.println("-->" + (efret.getMantissa().ToRadixString(2)) + " " +
        // efret.getExponent());
