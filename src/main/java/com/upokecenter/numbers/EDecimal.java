...
*/
  ...
  public final class EDecimal implements Comparable<EDecimal> {
    ...
    /**
     * Returns a number with the value {@code exponent*10^significand}.
    ...
    /**
     * Creates a number with the value {@code exponent*10^significand}.
     ...
     * floating-point number, use that object's {@code UnsignedMantissa} property.
...
@Deprecated
     ...
     * {@code longerValue} is 0 or greater, the return value will represent it. If
     * {@code longerValue} is less than 0, the return value will store 2^64 plus
     * this value instead.
     */
...
(1 << (EFloat.Binary16SignifAreaBits - 1))) != 0;
     ...
     * Creates an arbitrary-precision decimal number from a sequence of bytes
     * (interpreted as text) that represents a number. See {@code
     * FromString(string, int, int, EContext)} for more information.
     ...
     * {@code other} is null, or 0 if both values are equal. This implementation
     ...
     * {@code HasFlags} and {@code IsSimplified} of the context are true and only
     ...
     * {@code EContext} (such as {@code EContext.Decimal128}) instead.
     */
...
@Deprecated
     ...
     * left. If this number is negative, instead moves the decimal point to the
     * right by this number's absolute value.
     ...
     * the right.
     * @param places The number of decimal places to move the decimal point to the
     * right. If this number is negative, instead moves the decimal point to the
     * left by this number's absolute value.
     ...
     * arbitrary-precision decimal floating-point number and returns the result.
     * The exponent for the result is this arbitrary-precision decimal
     ...
     * other 64-bit signed integer's exponent.
     * @param longValue The parameter {@code longValue} is a 64-bit signed integer.
     * @return The product of the two numbers, that is, this arbitrary-precision
     ...
     * this arbitrary-precision decimal floating-point number's exponent plus the
     * other 32-bit signed integer's exponent.
     ...
     * adjustment is done only once, namely, after multiplying and adding.
     ...
     * precision is unlimited and rounding isn't needed. If the precision doesn't
     * indicate a simplified arithmetic, rounding and precision.Divide(exponent)
     * adjustment is done only once, namely, after multiplying and subtracting.
     ...
     * method does not necessarily return a copy of this object.).
     */
     ...
     * value if both values are equal.
     ...
     * precision. If {@code ctx} is null or the precision and exponent range are
     * unlimited, returns the same value as this object (or a quiet NaN if this
     * object is a signaling NaN).
     */
     ...
     * desired exponent of 3: {@code 10e3}, {@code 8888e3}, {@code 4.56e5}.
     * @param ctx An arithmetic context to control precision and rounding of the
     * result. If {@code HasFlags} of the context is true, will also store the
     * flags resulting from the operation (the flags are in addition to the
     * pre-existing flags). Can be null, in which case the default rounding mode is
     ...
     * still be some trailing zeros in the significand.
     */
     ...
     * @param divisor The number to divide by.
     * @param ctx An arithmetic context object to control the precision, rounding,
     * and exponent range of the result, and of the intermediate integer division.
     * If {@code HasFlags} of the context is true, will also store the flags
     * resulting from the operation (the flags are in addition to the pre-existing
     * flags). Can be null, in which the precision is unlimited.
     ...
     * @param divisor The number to divide by.
     * @param ctx An arithmetic context object to control the precision, rounding,
     * and exponent range of the result, but not also of the intermediate integer
...
@Deprecated
     ...
     * precision is unlimited and no rounding is needed.
     * @return The closest value to this object's value, rounded to the specified
     * precision. Returns the same value as this object if {@code ctx} is null or
     ...
     * operation per the "simplified" arithmetic given in Appendix A of the General
     ...
     * if {@code places} is 5, "78E-2" becomes "78E+3" and has a bigger value.
     */
     ...
     * For example, in general, if {@code bigPlaces} is 5, "78E-2" becomes "78E+3"
     * and has a bigger value.
     ...
     * (the context's Precision property is 0).
     */
...
@Deprecated
     ...
     * the IEEE 754 binary64 format. The return value can be positive infinity or
     * negative infinity, encoded in the IEEE 754 binary64 format, if this value
     ...
     * fractional part of the value would be lost. Note that depending on the
     ...
     * range; that method will throw an exception on overflow.
     * @return An arbitrary-precision integer.
     * @throws ArithmeticException This object's value is infinity or not-a-number
     * (NaN).
...
@Deprecated
     ...
     * value, using the half-even rounding mode.
...
(1 << (EFloat.Binary32Bits - 1)))) :
     ...
     * exponential notation (expressed as a number 1 or greater, but less than 10,
     * times a power of 10) if this object's Exponent property is greater than 0 or
     ...
     * this number is infinity or not-a-number (NaN).
     * @return An arbitrary-precision decimal number.
     */
    public EDecimal Ulp() {
     ...
     * this parameter is {@code EContext.Binary64}. Can be null.
